...
### 4.6 错误提示：访问域名返回“服务器 80 端口内容” (默认页/Welcome to nginx)
如果你配置了 Nginx 反向代理，但访问域名时显示的不是 API 返回的内容，而是 Nginx 的默认欢迎页或“站点创建成功”页面，说明请求**没有被代理到 Node 后端**。

**可能原因与解决方法**：

1. **配置未生效 (最常见)**：
   宝塔修改 Nginx 配置后需要重载。点击宝塔软件商店 -> Nginx -> 服务 -> **重载配置** (Reload configuration)。
2. **后端未启动**:
   虽然通常会报 502 Bad Gateway，但如果配置有误（如 `location` 匹配问题），可能会漏到默认页。
   **验证方法**：在服务器终端运行 `curl http://127.0.0.1:3002`。如果能看到 "API server" 或 JSON，说明后端正常，问题在 Nginx。
3. **域名解析/绑定问题**:
   确认你的域名 `launchapi.ljcode.cn` 确实解析到了该服务器 IP，且在宝塔的 **网站** 列表中，该域名的状态是 **运行中**。
4. **默认站点干扰**:
   如果服务器有设置“默认站点” (Default Server)，且你的域名没有正确匹配到 Server Block，就会显示默认站点的内容。确保配置文件中的 `server_name` 填写无误。
5. **Node 项目与纯静态冲突**:
   在宝塔中，如果你即添加了“Node 项目”又添加了同域名的“PHP/HTML 网站”，可能会冲突。建议只保留一个。如果是通过“Node 项目”添加的，其内置的 Nginx 配置通常已经包含了 proxy_pass 到指定端口。
6. **配置文件检查**:
   确保你的 `location /` 块中包含 `proxy_pass http://127.0.0.1:3002;`。你提供的配置看起来是正确的，请重点检查是否重载了 Nginx。
